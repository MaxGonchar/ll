...
def assert_access_token_in_response(response: dict, user_id: str):
    assert "access_token" in response
    assert_token_content(response["access_token"], user_id)
    assert response["access_token"] is not None


def assert_refresh_token_in_response(response: dict, user_id: str):
    assert "refresh_token" in response
    assert_token_content(response["refresh_token"], user_id)
    assert response["access_token"] is not None


def assert_token_content(token: str, user_id: str):
...
def assert_user_obj(user_obj: User, user: User):
...
def test_register():
    ...
    with (
        patch("src.controllers.auth_controllers.repo.add_user") as mock_add_user,
        patch("src.controllers.auth_controllers.uuid4", return_value=user_id),
        patch("src.controllers.auth_controllers.get_current_time", return_value=last_login),
        app.app_context(),
    ):
